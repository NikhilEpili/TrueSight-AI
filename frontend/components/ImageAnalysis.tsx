...
export default function ImageAnalysis() {
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      setError(e.message || 'Analysis failed. Please check if the backend server is running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#23243a] via-[#181824] to-[#2d2c3a] py-12 px-4">
      <div className="relative w-full max-w-2xl mx-auto bg-white/10 rounded-2xl shadow-xl p-10 flex flex-col items-center">
        <h1 className="text-3xl md:text-4xl font-extrabold mb-4 bg-gradient-to-r from-purple-400 to-indigo-500 bg-clip-text text-transparent text-center font-poppins">Image Analysis</h1>
        <p className="text-lg text-white mb-8 text-center font-rubik">Upload your image to detect if it's AI-generated or authentic human content</p>
