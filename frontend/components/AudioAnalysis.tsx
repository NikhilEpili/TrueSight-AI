...
export default function AudioAnalysis() {
  ...
  const onAnalyze = async () => {
    ...
    setResult(null);
    try {
      const res = await APIService.analyzeAudio(selectedFile);
      setResult(res);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#23243a] via-[#181824] to-[#2d2c3a] py-12 px-4">
      <div className="relative w-full max-w-2xl mx-auto bg-white/10 rounded-2xl shadow-xl p-10 flex flex-col items-center">
        <h1 className="text-3xl md:text-4xl font-extrabold mb-4 bg-gradient-to-r from-purple-400 to-indigo-500 bg-clip-text text-transparent text-center font-poppins">Audio Analysis</h1>
        <p className="text-lg text-white mb-8 text-center font-rubik">Upload your audio file to detect if it's AI-generated or authentic human content</p>
        <div className="w-full flex flex-col items-center mb-8">
          ...
          <label htmlFor="audio-upload" className="w-full flex flex-col items-center justify-center border-2 border-dashed border-purple-400 rounded-xl py-8 cursor-pointer hover:border-indigo-400 transition">
            <span className="text-lg text-indigo-300 font-semibold mb-2 font-rubik">Choose Audio File +</span>
            <input id="audio-upload" type="file" accept="audio/*" className="hidden" onChange={onFileChange} />
          </label>
        </div>
        <button onClick={onAnalyze} disabled={!selectedFile || loading} className="w-full bg-purple-500 disabled:bg-purple-800 disabled:opacity-60 text-white font-semibold rounded-lg px-6 py-2 text-lg mb-4 transition-transform transition-colors duration-200 hover:bg-purple-700 hover:scale-105 font-poppins">{loading ? 'Analyzing...' : 'Analyze 🔗'}</button>
          ...
          <div className="w-full mt-2 p-4 bg-white/5 rounded-lg border border-purple-400 text-white text-sm">
            <div className="mb-2">
              <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                ...
                {result.classification === 'fake' ? '🚨 DEEPFAKE DETECTED' : '✅ AUTHENTIC'}
              </span>
            </div>
